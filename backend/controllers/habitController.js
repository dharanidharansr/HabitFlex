...
const { scheduleReminders, cancelReminders } = require("../services/reminderService");
const {
...
// Get a single habit for copying (returns safe data without user-specific info)
exports.getHabitForCopy = async (req, res) => {
  try {
    const habit = await Habit.findById(req.params.id)
      .populate("user", "username") // Only get username for attribution
      .select("-completedDates -currentStreak -longestStreak"); // Exclude user-specific progress

    if (!habit) {
      return res.status(404).json({ message: "Habit not found" });
    }

    // Return habit details that are safe to copy
    res.json({
      _id: habit._id,
      name: habit.name,
      ...
      user: habit.user, // For attribution
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.updateHabit = async (req, res) => {
  try {
    ...
    // Reschedule reminders if time or reminder settings changed
    if (req.body.timeOfDay || req.body.reminderSettings) {
      console.log(`Rescheduling reminders for habit: ${updatedHabit.name}`);
...
exports.deleteHabit = async (req, res) => {
  try {
    ...
    console.log(`Cancelling reminders for habit: ${habit.name}`);
