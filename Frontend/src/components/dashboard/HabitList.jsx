...
const HabitList = ({ habits, onHabitUpdate, user }) => {
  ...
  const handleCheck = async (habitId) => {
    ...
    try {
      ...
      console.log('🚀 Making API call to complete habit...');
      ...
      if (response.data.completedToday) {
        console.log('✅ Habit marked as completed! Firing confetti...');
        fire();
        setJustCompleted(habitId);
        setTimeout(() => setJustCompleted(null), 2000);

        // Optimistically update UI
        console.log('🔄 Calling onHabitUpdate to refresh data...');
        onHabitUpdate();
        toast.success("Habit completed! 🎉");
      } else {
        console.log('⚠️ API returned completedToday as false');
      }
    } catch (error) {
