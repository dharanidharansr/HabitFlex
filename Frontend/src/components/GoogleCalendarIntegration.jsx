...
const GoogleCalendarIntegration = () => {
  ...
  React.useEffect(() => {
    ...
    if (user && user.googleCalendar) {
      setConnected(user.googleCalendar.connected || false);
    }
    ...
    const checkConnection = async () => {
      try {
        ...
        const isConnected = response.data.googleCalendar?.connected || false;
        setConnected(isConnected);

        // Update localStorage if it differs from backend
        if (user && user.googleCalendar && user.googleCalendar.connected !== isConnected) {
          user.googleCalendar.connected = isConnected;
